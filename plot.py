...
def create_plot_with_fixed_metric(all_data, lims, raw, x_scale, y_scale, xn, yn, fn_out, linestyles, fix_param, fix_min, fix_max):
    """
    Create plot with fixed metric range filtering.
    Filters algorithms to only show configurations within specified metric bounds.
    """
    ...
    # Create descriptive title showing the filtering constraints
    description = ""
...
def create_plot_cdf(all_data_original, all_cdfs, raw, xn, yn, fn_out, linestyles, fix_recall=0.95):
    """
    Create robustness CDF plot showing how algorithms perform across different recall thresholds.
    Plots the cumulative distribution function of recall rates for each algorithm.
    """
    ...
    # Step 1: Find the best configuration for each algorithm at the fixed recall level
    for algo in all_data_original.keys():
        # Select configuration that achieves the target recall rate
        ...
        # Step 2: Extract CDF data for the selected configuration
    ...
    # Plot CDF curves for each algorithm
    for cdf_data in selected_cdf_data:
        xs = np.array(cdf_data[-2])  # Recall thresholds (x-axis)
        ys = np.array(cdf_data[-1])  # CDF values (y-axis)
        algo = cdf_data[0]
    ...
    # Step 4: Configure plot appearance
    ax = plt.gca()
...
if __name__ == "__main__":
    ...
    # Plot type selection: controls which type of visualization to generate
    parser.add_argument(
        "-T", 
        "--plot-type", 
    ...
    # CDF plot parameters: for robustness analysis
    parser.add_argument("--fix-recall", help="Fix the recall value for robustness figure", default=90)
    
    # Filter plot parameters: for metric-based filtering
    parser.add_argument(
        "--fix-metric", 
    ...
    # Main plotting logic: choose plot type based on user arguments
    if args.plot_type == "cdf":
        # CDF plot: Shows robustness across different recall thresholds
        # Load results and compute CDF data for robustness analysis
        results = load_all_results(args.dataset, count, neurips23track=args.neurips23track)
        ...
        # Filter plot: Shows algorithms filtered by a specific metric range
        # Load results and compute metrics for filtering
        results = load_all_results(args.dataset, count, neurips23track=args.neurips23track)
        ...
        # Normal plot: Standard 2D plot of two metrics
        create_plot(
